#!/usr/bin/env bash
usage() { echo "$(basename $0) [-f tractoflow_input] [-c civet_nf_output] [-o output]" 1>&2; exit 1; }
...
do
    ...
    # CIVET input
    for cfile in t1.nii.gz pve_csf.nii.gz pve_gm.nii.gz pve_sc.nii.gz pve_wm.nii.gz brain_mask.nii.gz;
    do
        if [ -f ${CIVETNFOUTDIR}/${i}/Register_civet_to_T1/*${cfile} ]; then
            cp -L ${CIVETNFOUTDIR}/${i}/Register_civet_to_T1/*${cfile}  ${OUTDIR}/${i}/${cfile}
        else
